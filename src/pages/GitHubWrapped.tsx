...
const GitHubWrapped = () => {
  ...
  const highlights = [
    {
      ...
      stars: 12,
      desc: "Problem solving repository",
    },
  ...
  const achievements = [
    { title: "SLAKDEV", date: "2025", desc: "Burst-style Development" },
    {
      ...
      date: "July",
      desc: "115 Contributions in July",
    },
    {
  ...
  return (
    <article
      className="min-h-screen bg-[#0d1117] text-gray-300 font-sans md:p-6 relative overflow-hidden cursor-none selection:bg-green-900 selection:text-white"
      style={{ "--cursor-color": "#ffffff" } as React.CSSProperties}
    >
      <SEO
        title="GitHub Wrapped"
        description="My GitHub Year in Review 2025."
      />
      <Cursor />

      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-green-900/10 via-[#0d1117] to-[#0d1117] pointer-events-none" />
      ...
      >
        ...
        <motion.div
          ...
          className="md:col-span-8 bg-gradient-to-br from-[#2ea44f] to-[#238636] rounded-3xl p-8 text-white shadow-[0_0_30px_rgba(46,164,79,0.2)] md:min-h-[300px] flex flex-col justify-between relative overflow-hidden group"
        >
          <div className="absolute top-0 right-0 text-[200px] opacity-10 rotate-12 translate-x-10 -translate-y-10 pointer-events-none">
            <FaGithub />
          </div>
          <div>
          ...
          <p className="relative z-10 max-w-md mt-6 text-green-50 text-lg leading-relaxed">
            You build with bursts of brilliance. With{" "}
        ...
        >
          {[
            ...
            {
              icon: <GoIssueOpened className="text-3xl text-[#3fb950] mb-2" />,
              value: stats.issues,
              label: "Issues Solved",
            },
            {
            ...
            {
              icon: <FaTerminal className="text-3xl text-orange-400 mb-2" />,
            ...
            <div
              key={i}
              className="bg-[#161b22] border border-gray-800 rounded-3xl p-5 flex flex-col justify-center items-center text-center hover:bg-gray-800/50 transition-colors"
            >
              {item.icon}
        ...
        <motion.div
          variants={itemVariants}
          className="md:col-span-4 bg-[#161b22] border border-gray-800 rounded-3xl p-6"
        >
        ...
        >
          ...
          <div className="space-y-3">
            {highlights.map((repo, i) => (
              <div
                key={i}
                className="bg-[#0d1117] border border-gray-800 rounded-xl p-3 hover:border-gray-600 transition-colors cursor-pointer group"
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-blue-400 group-hover:underline text-sm truncate">
                    {repo.name}
                  </span>
                  <span className="text-xs text-gray-500 flex items-center gap-1 shrink-0 ml-2">
                    <FaStar className="text-xs" /> {repo.stars}
                  </span>
                </div>
                <p className="text-xs text-gray-400">{repo.desc}</p>
              </div>
            ))}
          </div>
        </motion.div>
