...
const LeetCodeWrapped = () => {
  ...
  const stats = {
    ...
    maxStreak: 100,
    topTopics: [
      { name: "Arrays & Strings", count: 65, color: "bg-blue-500" },
  ...
  const ratingHistory = [
    1500, 1540, 1600, 1580, 1650, 1720, 1700, 1780, 1820, 1856, 1840, 1880,
  ];
  ...
  return (
    <article
      className="min-h-screen bg-[#1a1a1a] text-gray-300 font-sans md:p-6 relative overflow-hidden cursor-none selection:bg-orange-500 selection:text-white"
      style={{ "--cursor-color": "#ffffff" } as React.CSSProperties}
    >
      <SEO
        title="LeetCode Wrapped"
        description="My LeetCode Year in Review 2025."
      />
      <Cursor />

      <div className="fixed top-20 right-20 w-96 h-96 bg-orange-500/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="fixed bottom-20 left-20 w-96 h-96 bg-yellow-500/5 rounded-full blur-[100px] pointer-events-none" />
      ...
      >
        ...
        <motion.div
          ...
          className="md:col-span-8 bg-gradient-to-r from-[#2c2c2c] to-[#1e1e1e] border border-[#333] rounded-3xl p-8 flex flex-col justify-between shadow-2xl relative overflow-hidden group"
        >
          <div className="absolute top-0 right-0 text-[200px] opacity-5 text-[#FFA116] rotate-12 translate-x-10 -translate-y-10 group-hover:rotate-[20deg] transition-transform duration-700 pointer-events-none">
            <SiLeetcode />
          </div>
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#FFA116]/10 text-[#FFA116] rounded-full text-xs font-bold mb-4 border border-[#FFA116]/20">
              <FaTrophy /> RANK: {stats.globalRank}
            </div>
            ...
            <div className="mt-6 flex gap-6 text-sm text-gray-400">
              <div className="flex items-center gap-2">
                <FaCode className="text-cyan-400" /> {stats.submissions}{" "}
                Submissions
              </div>
              <div className="flex items-center gap-2">
        ...
        <motion.div
          variants={itemVariants}
          className="md:col-span-4 bg-[#262626] border border-[#333] rounded-3xl p-8 flex flex-col justify-between hover:border-[#444] transition-colors relative group"
        >
        ...
        <motion.div
          variants={itemVariants}
          className="md:col-span-12 bg-[#262626] border border-[#333] rounded-3xl p-6 relative overflow-hidden"
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
            {stats.badges.map((badge, i) => (
              <div
                key={i}
                className="bg-[#333] rounded-2xl p-4 flex flex-col items-center text-center gap-2 hover:bg-[#444] transition-colors group cursor-pointer"
              >
        ...
        <motion.div
          variants={itemVariants}
          className="md:col-span-4 bg-[#262626] border border-[#333] rounded-3xl p-6"
        >
          <h3 className="text-gray-100 font-bold mb-6">Difficulty</h3>
          <div className="space-y-6">
            {[
              {
                label: "Easy",
                count: stats.solved.easy,
                color: "bg-cyan-500",
                textColor: "text-cyan-500",
              },
              {
                label: "Med",
                count: stats.solved.medium,
                color: "bg-yellow-500",
                textColor: "text-yellow-500",
              },
              {
                label: "Hard",
                count: stats.solved.hard,
                color: "bg-red-500",
                textColor: "text-red-500",
              },
            ].map((d) => (
              <div key={d.label} className="flex items-center gap-4">
                <div className={`w-16 ${d.textColor} font-bold text-sm`}>
                  {d.label}
                </div>
                <div className="flex-1 h-3 bg-[#333] rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${(d.count / stats.solved.total) * 100}%`,
                    }}
            ...
            {stats.topTopics.map((topic, i) => (
              <div key={i}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-mono text-[#FFA116]">
                    {topic.count}
                  </span>
                </div>
                <div className="h-2 bg-[#333] rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${(topic.count / 150) * 100}%` }}
        ...
        >
          <div className="flex-1 bg-[#262626] border border-[#333] rounded-3xl p-6 flex flex-col relative overflow-hidden">
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div className="text-green-500 text-xs font-bold bg-green-500/10 px-2 py-1 rounded">
                +12.4%
              </div>
            </div>
