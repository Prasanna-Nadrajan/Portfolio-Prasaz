...
const services = [
  {
    ...
    description:
      "Uncovering insights to drive impactful, data-driven decisions",
    icon: "/assets/images/mp_da.png",
  },
  {
    title: "MERN Stack",
    description:
      "Building scalable full-stack web applications using MongoDB, Express, React, and Node.js.",
  ...
  },
  {
    ...
    description:
      "Developing intelligent models to predict trends and automate complex tasks.",
...
const testimonials = [
  ...
  {
    name: "Chennai Data Circle - AI DAY @ Ford",
    date: "Feb 21, 2026",
...
const About = () => {
  ...
  return (
    <article className="about active animate-fade-in" data-page="about">
      ...
      <header>
        <h2 className="h2 article-title text-2xl font-semibold mb-4 border-b-2 border-neon-blue w-max pb-1">
          About me
        </h2>
      </header>
      ...
      <section className="about-text text-secondary-text leading-relaxed mb-8">
        <p className="mb-4 text-justify">
          Hey! I work with data. Essentially, I'm a bit of a data detective—I
          spend my time digging through numbers and information to find hidden
          patterns and stories. It’s amazing what you can uncover; you might
          find the real reason a certain campaign took off, or pinpoint the
          exact moment a process starts to slow down. But it's not just about
          finding these interesting facts for their own sake. I then use what I
          find to help make better, smarter decisions.
        </p>
        <p className="mb-4 text-justify">
          My role is to translate all that complex information into a clear and
          simple recommendation, so instead of just having a pile of numbers, we
          have a solid next step. Ultimately, I help the team move forward with
          confidence, knowing our choices are backed by actual evidence, not
          just guesswork.
        </p>
      </section>

      <section className="service mb-8">
        <h3 className="h3 service-title text-xl font-semibold mb-5">
          What i'm doing
        </h3>
      ...
      <section className="testimonials mb-8">
        <h3 className="h3 testimonials-title text-xl font-semibold mb-5">
          Favourites
        </h3>
        <ul className="testimonials-list grid grid-cols-1 md:grid-cols-2 gap-6">
          {(showAllEvents ? testimonials : testimonials.slice(0, 4)).map((item, index) => (
            <li
              key={index}
              data-cursor="hover"
              className="testimonials-item bg-container-bg border border-border-color shadow-sm p-8 rounded-3xl relative z-10 cursor-pointer hover:border-neon-blue/50 hover:shadow-lg hover:shadow-neon-blue/20 hover:scale-[1.02] transition-all duration-300 h-full"
              onClick={() => openModal(item)}
            >
              <div className="content-card flex flex-col h-full">
                <figure className="testimonials-avatar-box w-full h-48 rounded-2xl overflow-hidden mb-4 shadow-sm shrink-0">
                  <img
                    src={item.avatar}
                    alt={item.name}
                ...
                <div className="testimonials-content flex flex-col flex-grow">
                  <time className="text-xs text-neon-blue font-medium mb-2 block">
                    {item.date}
                  </time>
                  <h4 className="h4 testimonials-item-title text-lg font-medium text-main-text mb-2 leading-snug">
                    {item.name}
                  </h4>
